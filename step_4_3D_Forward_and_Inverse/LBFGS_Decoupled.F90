...
! *************************************************************************************************
           ...
           Else
! 2.1- LBFGS k
             K_LBFGS = Iter - Iter_begin ! (= Iter - 1)
...
! Lambda
             ...
             rho_Lambda_LBFGS = 1.0d0 / rho_Lambda_LBFGS
...
! Mu
             ...
             rho_Mu_LBFGS = 1.0d0 / rho_Mu_LBFGS

!2.3- warn user if rho is large
             Tol_rho_LBFGS = 1.0d30
! Lambda
             If ( (Dabs (rho_Lambda_LBFGS) > Tol_rho_LBFGS) .AND. (Control_Parameter /= 'Mu') ) Then
                If ( rank == 0 ) Then
                   Write(*    ,'(A33,E33.10E3)') 'rho_Lambda_LBFGS = ', rho_Lambda_LBFGS
                   Write(UnInf,'(A33,E33.10E3)') 'rho_Lambda_LBFGS = ', rho_Lambda_LBFGS
                End If
             End If
! Mu
             If ( (Dabs (rho_Mu_LBFGS) > Tol_rho_LBFGS) .AND. (Control_Parameter /= 'Lambda') ) Then
                If ( rank == 0 ) Then
                   Write(*    ,'(A33,E33.10E3)') 'rho_Mu_LBFGS = ', rho_Mu_LBFGS
                   Write(UnInf,'(A33,E33.10E3)') 'rho_Mu_LBFGS = ', rho_Mu_LBFGS
                End If
             End If
...
! 2.3.2- where to store the latest vectors                
                ...
                End If
! 2.3.3- store scalars
                Rhos_Lambda_LBFGS ( N_store ) = rho_Lambda_LBFGS
                    Rhos_Mu_LBFGS ( N_store ) =     rho_Mu_LBFGS
...
! 2.3.8- loop 1 =================================================================
                Do I_vec = 1 , N_use
                   ...
                   ! Lambda
                   Alphas_Lambda_LBFGS_Rank ( I ) = Rhos_Lambda_LBFGS ( I ) * Dot_Product ( q_Lambda (:) , Ss_Lambda_LBFGS ( : , I ) )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   Call MPI_Allreduce ( Alphas_Lambda_LBFGS_Rank ( I ), Alphas_Lambda_LBFGS ( I ), 1, MPI_DOUBLE_PRECISION, MPI_SUM, PETSC_COMM_WORLD, IERR )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   q_Lambda (:) = q_Lambda (:) - Alphas_Lambda_LBFGS ( I ) * Ys_Lambda_LBFGS ( : , I )
                   ! Mu
                   Alphas_Mu_LBFGS_Rank ( I ) = Rhos_Mu_LBFGS ( I ) * Dot_Product ( q_Mu (:) , Ss_Mu_LBFGS ( : , I ) )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   Call MPI_Allreduce ( Alphas_Mu_LBFGS_Rank ( I ), Alphas_Mu_LBFGS ( I ), 1, MPI_DOUBLE_PRECISION, MPI_SUM, PETSC_COMM_WORLD, IERR )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   q_Mu (:) = q_Mu (:) - Alphas_Mu_LBFGS ( I ) * Ys_Mu_LBFGS ( : , I )
                End Do
! 2.3.9- H^0 ====================================================================
                ! Lambda
                ...
                r_Lambda = ( H0_Lambda_numerator / H0_Lambda_denominator ) * q_Lambda
                ! Mu
                ...
                r_Mu = ( H0_Mu_numerator / H0_Mu_denominator ) * q_Mu
! 2.3.10- loop 2 ================================================================
                Do I_vec = N_use, 1, -1
                   I = LBFGS_vector_sequence ( I_vec )
                   ! Lambda
                   beta_Lambda_LBFGS_Rank = Rhos_Lambda_LBFGS ( I ) * Dot_Product ( r_Lambda (:) , Ys_Lambda_LBFGS ( : , I ) )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   Call MPI_Allreduce ( beta_Lambda_LBFGS_Rank, beta_Lambda_LBFGS, 1, MPI_DOUBLE_PRECISION, MPI_SUM, PETSC_COMM_WORLD, IERR )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   r_Lambda (:) = r_Lambda (:) + Ss_Lambda_LBFGS ( : , I ) * ( Alphas_Lambda_LBFGS ( I ) - beta_Lambda_LBFGS )
                   ! Mu
                   beta_Mu_LBFGS_Rank = Rhos_Mu_LBFGS ( I ) * Dot_Product ( r_Mu (:) , Ys_Mu_LBFGS ( : , I ) )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   Call MPI_Allreduce ( beta_Mu_LBFGS_Rank, beta_Mu_LBFGS, 1, MPI_DOUBLE_PRECISION, MPI_SUM, PETSC_COMM_WORLD, IERR )
                   Call MPI_Barrier ( PETSc_COMM_WORLD, IERR )
                   r_Mu (:) = r_Mu (:) + Ss_Mu_LBFGS ( : , I ) * ( Alphas_Mu_LBFGS ( I ) - beta_Mu_LBFGS )
                End Do
!                ================================================================
...
! watchdog
                Call VecGetLocalSize ( p_Lambda_PETSC, NJ_Mapping_from_call, IERR ) 
                If ( NJ_Mapping_from_call /= NJ_Mapping ) Then
                   Write(*,*) 'NJ_Mapping_from_call /= NJ_Mapping - 2.3.24'
                   Stop
                End If
