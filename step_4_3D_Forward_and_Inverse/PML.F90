...
!************************************************
! REVISION : T 18 February 2014: I have had more trouble with myself than with any other man.
...
!---------- ---------- ---------- ----------
      ...
      PML_alpha_beta (2) = PML_alpha_beta (2) + (PML_alpha0x * FAC1 ** PML_polynomial_degree) * ratio
      ! alpha_z
      PML_alpha_beta (3) = PML_alpha_beta (3) + (PML_alpha0x * FAC1 ** PML_polynomial_degree) * ratio
      ! beta_x

      ! beta_y
      PML_alpha_beta (5) = PML_alpha_beta (5) + (PML_beta0x  * FAC1 ** PML_polynomial_degree) * ratio
      ! beta_z
      PML_alpha_beta (6) = PML_alpha_beta (6) + (PML_beta0x  * FAC1 ** PML_polynomial_degree) * ratio


      ! d/dx alpha_y
      d_dx_alpha_y = (PML_alpha0x * PML_polynomial_degree * FAC4 * FAC1 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dx alpha_z
      d_dx_alpha_z = (PML_alpha0x * PML_polynomial_degree * FAC4 * FAC1 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dx beta_y
      d_dx_beta_y  = (PML_beta0x  * PML_polynomial_degree * FAC4 * FAC1 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dx beta_z
      d_dx_beta_z  = (PML_beta0x  * PML_polynomial_degree * FAC4 * FAC1 ** (PML_polynomial_degree - 1.0d0)) * ratio
...
!---------- ---------- ---------- ----------
      ...
      ! alpha_z
      PML_alpha_beta (3) = PML_alpha_beta (3) + (PML_alpha0y * FAC2 ** PML_polynomial_degree) * ratio
      ! beta_x
      PML_alpha_beta (4) = PML_alpha_beta (4) + (PML_beta0y  * FAC2 ** PML_polynomial_degree) * ratio
      ! beta_y

      ! beta_z
      PML_alpha_beta (6) = PML_alpha_beta (6) + (PML_beta0y  * FAC2 ** PML_polynomial_degree) * ratio


      ! d/dy alpha_x
      d_dy_alpha_x = (PML_alpha0y * PML_polynomial_degree * FAC5 * FAC2 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dy alpha_z
      d_dy_alpha_z = (PML_alpha0y * PML_polynomial_degree * FAC5 * FAC2 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dy beta_x
      d_dy_beta_x  = (PML_beta0y  * PML_polynomial_degree * FAC5 * FAC2 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dy beta_z
      d_dy_beta_z  = (PML_beta0y  * PML_polynomial_degree * FAC5 * FAC2 ** (PML_polynomial_degree - 1.0d0)) * ratio
...
!---------- ---------- ---------- ----------
      ...
      ! alpha_x
      PML_alpha_beta (1) = PML_alpha_beta (1) + (PML_alpha0z * FAC3 ** PML_polynomial_degree) * ratio
      ! alpha_y
      PML_alpha_beta (2) = PML_alpha_beta (2) + (PML_alpha0z * FAC3 ** PML_polynomial_degree) * ratio
      ! alpha_z

      ! beta_x
      PML_alpha_beta (4) = PML_alpha_beta (4) + (PML_beta0z  * FAC3 ** PML_polynomial_degree) * ratio
      ! beta_y
      PML_alpha_beta (5) = PML_alpha_beta (5) + (PML_beta0z  * FAC3 ** PML_polynomial_degree) * ratio
      ! beta_z


      ! d/dz alpha_x
      d_dz_alpha_x = (PML_alpha0z * PML_polynomial_degree * FAC6 * FAC3 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dz alpha_y
      d_dz_alpha_y = (PML_alpha0z * PML_polynomial_degree * FAC6 * FAC3 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dz beta_x
      d_dz_beta_x  = (PML_beta0z  * PML_polynomial_degree * FAC6 * FAC3 ** (PML_polynomial_degree - 1.0d0)) * ratio
      ! d/dz beta_y
      d_dz_beta_y  = (PML_beta0z  * PML_polynomial_degree * FAC6 * FAC3 ** (PML_polynomial_degree - 1.0d0)) * ratio
...
!if (rank == 0 ) then
!  write(999,'(3f20.3)') XG
!  write(999,'(3f20.3)') PML_alpha0x, PML_alpha0y, PML_alpha0z
!  write(999,'(3f20.3)') PML_beta0x, PML_beta0y, PML_beta0z
!  write(999,*) ''
!  write(999,'(20f20.3)') PML_alpha_beta(1:3)
!  write(999,'(20f20.3)') PML_alpha_beta(4:6)
!  write(999,'(20f20.3)') PML_alpha_beta(7:10)
!  write(999,'(20f20.3)') PML_alpha_beta(11:13)
!  write(999,'(20f20.3)') PML_alpha_beta(14:16)
!  write(999,'(20f20.3)') PML_alpha_beta(17:19)
!  write(999,*) ''
!  write(999,*) ''
!end if
